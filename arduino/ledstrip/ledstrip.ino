...
// ledstrip.ino — UNO R3: LED strip + buzzer (white-light rhythms) + RFID +
// microphone dB feedback Events (Serial in → display layer): ALARM_WARNING /
// ALARM_ON / HARD_OUT / MESSY_ON / MESSY_OFF / NOISY_ON / NOISY_OFF / QUIET_ON
// / END / RESET Events (Serial back → Hub): START / FINISH / NOISY_ON /
// NOISY_OFF / QUIET_ON

#define USE_NEOPIXEL \
  1  // 1=NeoPixel（WS2812/SK6812）；0=12V 單色燈條(PWM+MOSFET)
...
void ledsOff() {
  for (int i = 0; i < NUM_PIX; i++) strip.setPixelColor(i, 0);
  strip.show();
}
void ledsWhite() {
...
bool uidEq(byte *a, byte *b, byte len) {
...
bool noisy = false;
unsigned long noisyHit = 0, noisyFall = 0, quietAcc = 0;

float readDb() {
  long sumsq = 0;
  for (long i = 0; i < MIC_N; i++) {
    int v = analogRead(PIN_MIC) - 512;
    sumsq += (long)v * v;
    delayMicroseconds(MIC_US);
  }
  float rms = sqrt((float)sumsq / MIC_N);
  if (rms < 1) rms = 1;
...
void loop() {
  ...
  if (dB >= NOISY_DB) {
    if (!noisy) {
      if (noisyHit == 0) noisyHit = now;
      if (now - noisyHit >= (unsigned long)NOISY_ON_MS) {
        noisy = true;
        Serial.println("NOISY_ON");
      }
    }
    quietAcc = 0;
    noisyFall = 0;
  ...
  if (Serial.available()) {
    ...
    else if (cmd == "HARD_OUT") {
      buzzSoon();
      flash(2, 100, 100);
    }  // 快閃2 / quick double flash
    else if (cmd == "MESSY_ON") {
      buzzSoon();
      flash(3, 120, 120);
    }  // 閃3 / flash three times
    else if (cmd == "MESSY_OFF") {
      ledsOff();
    }  // 滅 / off
    else if (cmd == "NOISY_ON") {
      pulse(500);
    }  // 脈衝一次 / single pulse
    else if (cmd == "NOISY_OFF") {
      ledsOff();
    } else if (cmd == "QUIET_ON") {
      pulse(150);
    }  // 輕閃一次 / gentle pulse
    else if (cmd == "RFID_ERROR") {
      flash(2, 100, 100);
      buzz(150, 800);
    }  // 快閃+低音錯誤 / quick flash + low buzz
    else if (cmd == "END") {
      buzzFanfare();
      for (int i = 0; i < 3; i++) {
        pulse(180);
