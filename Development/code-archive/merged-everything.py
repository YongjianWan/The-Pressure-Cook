...
TOO_LOUD_DB = -20   # loud threshold
REL_DB  = -25   # lower bound for hysteresis of loud
TOO_QUIET_DB = -35  # quiet threshold
...
while True:
    ...
    elif (not task_due) and (not aruco_out) and last_avg_db >= TOO_LOUD_DB and PRIO_SOUND >= current_priority:
        now_ts = time.time()
    ...
    elif (not task_due) and (not aruco_out) and last_avg_db <= TOO_QUIET_DB and PRIO_SOUND >= current_priority:
        now_ts = time.time()
        if now_ts - last_volume_tts_ts >= VOLUME_TTS_COOLDOWN:
            last_volume_tts_ts = now_ts
            speak_and_blink("Volume is too quiet. Speak up",
                            "YELLOW_BLINK", times=5, delay=0.35,
                            priority=PRIO_SOUND, tag="sound")

    elif (not task_due) and (not aruco_out) and TOO_QUIET_DB < last_avg_db < TOO_LOUD_DB:
        # NORMAL volume → clear PRIO_SOUND only if blink + speech finished
        if current_priority == PRIO_SOUND and not blink_active and current_speech_tag != "sound":
            current_priority = 0
            send_led_state("GREEN")
