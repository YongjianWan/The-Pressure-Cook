// We acknowledge using ChatGPT and Claude AI in developing this code for Arduino and LEDs.
...
void enter(State s) {
  ...
  // Set timing for each blink mode
  switch (s) {
...
void handleSerial() {
  while (Serial.available()) {
    ...
    else if (command == "WHITE_BLINK")     enter(WHITE_BLINK);
  ...
  switch (ledState) {
    ...
    case WHITE_BLINK:
      if (now - lastToggle >= (ledOn ? blinkOn : blinkOff)) {
        ledOn = !ledOn;
        lastToggle = now;
        if (ledOn) fillColor(255, 255, 255);
        else { strip.clear(); strip.show(); }
      }
      break;
  }
