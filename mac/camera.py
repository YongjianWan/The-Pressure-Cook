...
MESSY_ON_S = (
    1.0  # 離位連續 ≥1.0s → MESSY_ON / outside allowed zones ≥1.0 s triggers MESSY_ON
)
MESSY_OFF_S = (
    0.8  # 回位連續 ≥0.8s → MESSY_OFF / back in allowed zone ≥0.8 s triggers MESSY_OFF
)
...
def open_cam(index=None, w=1280, h=720, fps=30, prefer_iphone=False):
    ...
    raise RuntimeError(
        f"no camera available; tried: {', '.join(tried) if tried else 'none'}"
    )
...
def in_any(pt, names) -> bool:
    for n in names:
        if n in SECTIONS and in_poly(pt, SECTIONS[n]):
            return True
    return False
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ArUco marker tracker with multi-ROI logic"
    )
    parser.add_argument(
        "--camera", help="Camera index or stream URL (overrides CAMERA_INDEX env)."
    )
    parser.add_argument(
        "--prefer-iphone",
        action="store_true",
        help="On macOS, try Continuity Camera/iPhone first.",
    )
    parser.add_argument(
        "--width", type=int, default=CAM_W, help=f"Capture width (default: {CAM_W})."
    )
    parser.add_argument(
        "--height", type=int, default=CAM_H, help=f"Capture height (default: {CAM_H})."
    )
    parser.add_argument(
        "--fps", type=int, default=CAM_FPS, help=f"Capture FPS (default: {CAM_FPS})."
    )
...
def main():
    ...
    try:
        cap = open_cam(
            cam_arg, args.width, args.height, args.fps, prefer_iphone=prefer_iphone
        )
    ...
    while True:
        ...
        if ids is not None:
            ...
            for i, mid in enumerate(ids):
                ...
                if in_any(pt, allow):
                    ...
                    if (
                        messy_state.get(mid, False)
                        and (now - good_since[mid]) >= MESSY_OFF_S
                    ):
                        messy_state[mid] = False
                        emit("MESSY_OFF")
                    bad_since.pop(("MESSY", mid), None)
                else:
                    key = ("MESSY", mid)
                    bad_since.setdefault(key, now)
                    if (
                        not messy_state.get(mid, False)
                        and (now - bad_since[key]) >= MESSY_ON_S
                    ):
                        messy_state[mid] = True
                        emit("MESSY_ON")
        ...
        for name, poly in POLY.items():
            cv2.polylines(
                frame, [poly], True, (0, 255, 0) if name != "TABLE" else (255, 0, 0), 2
            )
        if ids is not None:
